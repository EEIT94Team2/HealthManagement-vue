                ...
                >
                  <div class="left">
                    ...
                    <span v-if="editingCommentId !== comment.id">
                      {{ comment.content ?? comment.message ?? comment.text ?? JSON.stringify(comment) }}
                    </span>
                    <el-input
                      v-else
...
const filteredComments = (postId) => {
  ...
  return list.filter(c => (c.content || c.message || c.text)?.includes(keyword));
};
...
const startEdit = (comment) => {
  ...
  editedCommentContent.value = comment.content ?? comment.message ?? comment.text ?? '';
};
